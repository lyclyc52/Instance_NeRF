...
class Trainer:
    def __init__(self, args, rank=0, world_size=1, device_id=None, logger=None):
        ...
        if args.wandb and rank == 0:
            wandb.init(project='nerf-rpn-iccv', config=deepcopy(args))
    ...
    def init_datasets(self):
        ...
        if self.args.dataset != 'general':
            if self.args.dataset_split.endswith('npz'):
                ...
                with open(self.args.dataset_split) as split:
                    split_dict = json.load(split)
                    self.train_scenes = np.array(split_dict['train'])
                    self.test_scenes = np.array(split_dict['test'])
                    self.val_scenes = np.array(split_dict['val'])
