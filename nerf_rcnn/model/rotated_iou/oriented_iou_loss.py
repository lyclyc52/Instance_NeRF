...
def aabb2obb_3d(aabb: torch.Tensor):
    """
    Transform aabb's to obb representation (to compute iou matrix with obb gt).
    Args:
        aabb (torch.Tensor): (..., 6) [xmin, ymin, zmin, xmax, ymax, zmax]
    Returns:
        obb (torch.Tensor): (..., 7) [x, y, z, w, l, h, theta]
    """
    xmin = aabb[..., 0::6]
    ymin = aabb[..., 1::6]
    zmin = aabb[..., 2::6]
    ...
    w[rot_idx] = l_t[rot_idx]
    l[rot_idx] = w_t[rot_idx]
    theta[rot_idx] = np.pi/2

    obb = torch.cat([x, y, z, w, l, h, theta], dim=-1)
    return obb
